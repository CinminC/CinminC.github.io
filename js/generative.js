...
function setup() {
  ...
  if (windowWidth >= 1200) {
    ...
    textAlign(CENTER);
    text(
      "Move your mouse to start drawing on canvas",
      windowWidth / 2,
      windowHeight / 2 - 80
    );
    textSize(22);
    text(
      "Instead of creating a static main page, I decide to invite you to draw it with me !",
      windowWidth / 2,
      windowHeight / 2
    );
    text(
      "As for what will the dynamic work end up to be, maybe you will find out when browsing my profile :)",
      windowWidth / 2,
      windowHeight / 2 + 50
    );
  }
  ...
  if (windowWidth >= 1200) {
    for (var l = -60; l < 60; l += 30) {
      for (var j = -60; j < 60; j += 30) {
        ...
        var n2 = noise(
          (mouseX + l) * 5 * noiseScale,
          (mouseY + j) * 5 * noiseScale
        );
